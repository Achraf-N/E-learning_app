...
const UserDetailsModal = ({ user, isOpen, onClose, onUpdate, onDelete }) => {
  ...
  const loadUserCourses = async () => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.COURSES.LIST_ALL}?user_id=${user.id}`, {
        headers: {
      ...
      setUserCourses([
        { id: 1, title: 'Mathematics Advanced', progress: 85, grade: 'A', status: 'In Progress' },
        { id: 2, title: 'Physics Fundamentals', progress: 92, grade: 'A+', status: 'Completed' },
        { id: 3, title: 'Chemistry Basics', progress: 67, grade: 'B', status: 'In Progress' },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const loadUserProgress = async () => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.PROGRESS.GET_ALL}?user_id=${user.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
      ...
      // Mock data for demonstration
      setUserProgress([
        { lesson_id: 1, lesson_title: 'Calculus Introduction', completed: true, score: 95, date: '2025-08-15' },
        { lesson_id: 2, lesson_title: 'Linear Algebra', completed: true, score: 88, date: '2025-08-18' },
        { lesson_id: 3, lesson_title: 'Differential Equations', completed: false, score: null, date: null },
      ]);
    }
  };
  ...
  const handleDeleteUser = () => {
    if (window.confirm(`Are you sure you want to delete ${user.first_name || user.nom_utilisateur}?`)) {
      onDelete(user);
      onClose();
    }
  };
                        ...
                        <div>
                          ...
                          <dl className="space-y-3">
                            ...
                            <div>
                              ...
                              <dd className="text-sm text-gray-900">{user.first_name || user.nom_utilisateur}</dd>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <dd className={`text-sm ${user.is_verified ? 'text-green-600' : 'text-red-600'}`}>
                          ...
                          <dl className="space-y-3">
                            ...
                            {isStudent && (
                              <div>
                                ...
                                <dd className="text-sm text-gray-900">{user.semester || 'S1'}</dd>
                              </div>
                            )}
                            <div>
                              <dt className="text-sm font-medium text-gray-500">Account Status</dt>
                              <dd className={`text-sm ${
                                user.statut_compte === 'ACTIF' ? 'text-green-600' : 'text-red-600'
                              }`}>
                                {user.statut_compte || 'ACTIF'}
                              </dd>
                            </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    {userProgress.map(progress => (
                      <div key={progress.lesson_id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center space-x-3">
                          <div className={`w-3 h-3 rounded-full ${
                            progress.completed ? 'bg-green-500' : 'bg-gray-300'
                          }`}></div>
                          <span className="font-medium text-gray-900">{progress.lesson_title}</span>
                        </div>
                        <div className="flex items-center space-x-4">
                          {progress.score && (
                            <span className="text-sm text-gray-600">Score: {progress.score}%</span>
                          )}
                          {progress.date && (
